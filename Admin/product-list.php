<?php
 ...
 $obj = new DbConnection();
  ...
  if (isset($_GET) && @$_GET['product_name'] !='') {
    $product_name = $_GET['product_name'];
    $data = $obj->getData("SELECT * FROM products, categories, brands, countries WHERE categories.cat_id = products.category_id AND brands.brand_id = products.brand_id AND countries.country_id = products.menufecture_counrty AND products.product_name LIKE '%$product_name%' ");
   
  }
  else{

    $data = $obj->getData("SELECT * FROM products, categories, brands, countries WHERE categories.cat_id = products.category_id AND brands.brand_id = products.brand_id AND countries.country_id = products.menufecture_counrty");
  }
    ...
    <div class="menu">
        <ul class="list">
           <li class="header nav_color">MAIN NAVIGATION</li>
            ...
            <li class="active open"> <a href="javascript:void(0);" class="menu-toggle"><i class="material-icons">add_shopping_cart</i><span>Products</span> </a>
                <ul class="ml-menu">
                    <li><a href="form-product_entry.php">New product</a></li>
                    <li class="active"><a href="product-list.php">Product List</a></li>
    ...
    <div class="container-fluid">
        ...
        <div class="col-sm-6">
            <div class="form-group">
                <label for="pn"><h6 class="noor_h6"><span class="noor_span"></span> Search Product</h6></label><br>
                <form action="product-list.php" method="get">
                    <div class="btn-group dropup">
                        <input type="text" name="product_name" placeholder="Enter Product Name" class="btn bg-orange waves-effect">
                        <input type="submit" name="submit" value="search" class="btn bg-orange waves-effect dropdown-toggle"> <span class="caret"></span> <span class="sr-only">Toggle Dropdown</span> 
                    </div>
                </form>
            </div>
        </div>
                        ...
                        <div class="table-responsive">
                            <table id="example" class="table table-bordered table-striped table-hover dataTable">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td> <img src="uploads/products/<?php echo $value['product_image'] ? $value['product_image'] : 'no-image.jpg' ; ?>" width="50" height="50">  </td>
                                            <td> <?php echo $value['product_name']; ?> </td>
                                            <td> <?php echo $value['category_name']; ?> </td>
                                            <td> <?php echo $value['brand_name']; ?> </td>
                                            <td> <?php echo $value['product_code']; ?> </td>
                                            <?php if ($value['quantity'] > 0){ ?>
                                            <td style="background-color: green; color: #fff;">In stock</td>    
                                            <?php } else{ ?>
                                            <td style="background-color: orange; color: #fff;">Out fo stock</td>
                                            <?php } ?>
                                            <td> <?php $color = @implode(', ',json_decode($value['color'])); echo $color; ?> </td>
                                            <td> <?php $size = @implode(', ',json_decode($value['size'])); echo $size; ?> </td>
                                            <td> <?php echo $value['country_name']; ?> </td>
                                            <td> <?php echo $value['price']; ?> </td>
                                            <td> <?php echo $value['discount']; ?> </td>
                                            <td> <?php echo $value['quantity']; ?> </td>
                                            <td> <?php echo $value['status'] ? 'Enable' : 'Disable' ; ?> </td>
                                            <td> <?php echo $value['feature_product'] ? 'Featured' : 'Unfeatured' ; ?> </td>
                                            <td> <?php echo $value['trending_product'] ? 'Trending' : 'Not Trending' ; ?> </td>
                                            <td>
                                                <ul class="header-dropdown">
                                                    <li class="dropdown"> <a href="javascript:void(0);" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"> <div class="demo-google-material-icon"> <i class="material-icons">settings</i></div>
                                                        <ul class="dropdown-menu pull-right">
                                                            ...
                                                            <li><a href="product_update.php?id=<?php echo base64_encode($value['id']); ?>" style="color: green"><div class="demo-google-material-icon"> <i class="material-icons">create</i> <span class="icon-name">Edit</span> </div></a></li>

                                                            <li><a href="controller/productDeleteProcess.php?id=<?php echo $value['id']; ?>" style="color: red" onclick="return confirm('Delete All Data of the Row')"><div class="demo-google-material-icon"> <i class="material-icons">delete_forever</i> <span class="icon-name">Delete</span></div></a></li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                            </td>
